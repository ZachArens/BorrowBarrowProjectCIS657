...
class TSItemModel {
    ...
    fileprivate func createTSItems()
    {
        ...
        items.append(ToolShedItem(itemName: "Cordless Drill", owner: "Barrel Box", itemDescription: "Lithium charging, charger provided", reqYesNo: true, requirements: "Must provide pick up/drop off, does not come with bits, responsible for fixing any damage or replacement if needed.", photoURL: "drill", thumbnailURL: "drill", lentTo: nil))
        items.append(ToolShedItem(itemName: "Tile Saw", owner: "Barrel Box", itemDescription: "I will show you how to operate, 110v, up to 10\" tile", reqYesNo: true, requirements: "Requires water hookup, responsible for fixing any damage or replacement if needed.", photoURL: "tilesaw", thumbnailURL: "tilesaw", lentTo: nil))
        items.append(ToolShedItem(itemName: "Leafblower", owner: "Barrel Box", itemDescription: "gas powered backpack leaf blower", reqYesNo: true, requirements: "Must provide own gasoline, responsible for fixing any damage or replacement if needed.", photoURL: "leafblower", thumbnailURL: "leafblower", lentTo: nil))
    }
}
